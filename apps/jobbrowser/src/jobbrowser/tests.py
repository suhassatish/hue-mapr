...
class TestMapReduce2NoHadoop:
  ...
  def test_acls_job(self):
    response = self.c.get('/jobbrowser/jobs/job_1356251510842_0054')
    assert_true(can_view_job('user', response.context['job']))
    assert_true(can_modify_job('user', response.context['job']))

    assert_false(can_view_job('user_no', response.context['job']))
    assert_false(can_modify_job('user_no', response.context['job']))
