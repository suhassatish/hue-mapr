...
class LdapBackend(object):
  ...
  def add_ldap_config(self, ldap_config):
    if ldap_config.LDAP_URL.get() is None:
      ...
      setattr(self.settings, 'AUTH_LDAP_SERVER_URI', ldap_config.LDAP_URL.get())
    ...
    if desktop.conf.LDAP.SEARCH_BIND_AUTHENTICATION.get():
      ...
      if ldap_config.BIND_DN.get():
        ...
        setattr(self.settings, 'AUTH_LDAP_BIND_DN', bind_dn)
        bind_password = ldap_config.BIND_PASSWORD.get()
        setattr(self.settings, 'AUTH_LDAP_BIND_PASSWORD', bind_password)
      ...
      setattr(self.settings, 'AUTH_LDAP_USER_SEARCH', search_bind_results)
    else:
      ...
      if nt_domain is None:
        ...
        setattr(self.settings, 'AUTH_LDAP_USER_DN_TEMPLATE', pattern)
      else:
        # %(user)s is a special string that will get replaced during the authentication process
        setattr(self.settings, 'AUTH_LDAP_USER_DN_TEMPLATE', "%(user)s@" + nt_domain)

    # Certificate-related config settings
    if ldap_config.LDAP_CERT.get():
      setattr(self.settings, 'AUTH_LDAP_START_TLS', ldap_config.USE_START_TLS.get())
      ldap.set_option(ldap.OPT_X_TLS_REQUIRE_CERT, ldap.OPT_X_TLS_ALLOW)
      ldap.set_option(ldap.OPT_X_TLS_CACERTFILE, ldap_config.LDAP_CERT.get())
    else:
      setattr(self.settings, 'AUTH_LDAP_START_TLS', False)
