...
<script type="text/html" id="collection">
  ...
  <div class="control-group" data-bind="css: {'error': source.errors().length > 0}">
    <label for="source" class="control-label">${_("Source")}</label>
    <div class="controls">
      <select data-bind="options: sources, value: source" name="type"></select>
    </div>
  </div>

  <div data-bind="template: { 'name': source, afterRender: afterSourceRender }"></div>
</script>
...
<script type="text/html" id="hbase">

</script>

<script type="text/html" id="hive">
  <div class="control-group" data-bind="css: {'error': hive.database.errors().length > 0}">
    <label for="name" class="control-label">${_("Database")}</label>
    <div class="controls">
      <select data-bind="options: hive.databases, optionsText: function(database) {return database.name()}, value: hive.database" name="type"></select>
    </div>
  </div>

  <!-- ko if: hive.database() -->
  <div class="control-group" data-bind="css: {'error': hive.database().table.errors().length > 0}">
    <label for="name" class="control-label">${_("Table")}</label>
    <div class="controls">
      <select data-bind="options: hive.database().tables, optionsText: function(table) {return table.name()}, value: hive.database().table" name="type"></select>
    </div>
  </div>
  <!-- /ko -->
</script>

<script type="text/html" id="separated">
...
<script type="text/html" id="morphlines">
...
function validateNamePage() {
  ...
  if (vm.source() == 'file') {
    ...
    vm.wizard.getPage('name').next(vm.fileType());
  } else if (vm.source() == 'hbase') {
    
  } else if (vm.source() == 'hive') {
    vm.inferFields($.map(vm.hive.database().table().columns(), function(column) { return [[column, 'string']]; }));
...
var root = vm.wizard.getPage('name', 'collection', 'separated', validateNamePage);
vm.wizard.getPage('separated', 'separated', 'fields', validateFetchFields);
vm.wizard.getPage('morphlines', 'morphlines', 'fields', validateFetchFields);
vm.wizard.getPage('fields', 'fields', null, validateFields);
