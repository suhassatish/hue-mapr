    ...
    <div class="card card-small">
      <ul class="nav nav-pills hueBreadcrumbBar" id="breadcrumbs">
        ...
        <span class="pull-right">
		  <button type="button" class="btn btn-primary">${ _('Upload jar') }</button>
        </span>        
      </ul>
    </div>
  </div>
...
<div id="uploadJarModal" class="modal hide fade">
  <div class="modal-header">
    <a href="#" class="close" data-dismiss="modal">&times;</a>
    <h3>${_('Upload Spark jars')}</h3>
  </div>
  <div class="modal-body">
	  <form class="form-horizontal" id="uploadJarForm" action="" method="POST" enctype="multipart/form-data">          
	    <div class="control-group">
	        <label class="control-label">Local jar file</label>
	      <div class="controls">                
	        <input type="file" name="jar_file" id="jar_file">
	      </div>
	    </div>
	    <input class="btn btn-primary" type="submit" value="${ _('Upload jar') }">
	  </form>
  </div>
  <div class="modal-footer">
    <button class="btn" data-dismiss="modal">${_('Cancel')}</button>
    <button data-bind="click: modalUploadJar" class="btn btn-primary">${_('Upload')}</button>
  </div>
</div>
