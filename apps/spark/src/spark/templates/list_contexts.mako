...
<div class="container-fluid">
  <div class="card card-small">
    ...
    <tbody>
      ...
      <tr>
        <td>${ contextz.get('status') }</td>
        <td>${ contextz.get('jobId') }</td>
        <td>${ contextz.get('classPath') }</td>
        <td>${ contextz.get('context') }</td>
        <td>${ contextz.get('startTime') }</td>
        <td>${ contextz.get('duration') }</td>
      </tr>
      % endfor
...
<script type="text/javascript" charset="utf-8">
  $(document).ready(function () {
    var viewModel = {
        availableSavedQueries : ko.observableArray(${ contexts | n,unicode }),
