...
function reinitializeTable () {
  window.setTimeout(function(){
    if ($(".dataTables_wrapper").height() > 100) {
      ...
      $($("a[data-toggle='tab']").parent(".active").find("a").attr("href")).height('auto');
    }
  }, 400)
}
...
var checkSetReady = (function() {
  var counter = 0;
  var stop = (viewModel.design.id() > 0) ? 2 : 1;
  return function() {
    if (++counter == stop) {
...
var fetchQuerySubscription = viewModel.databases.subscribe(function() {
  if (viewModel.design.id() > 0) {
    viewModel.fetchDesign();
  }
  fetchQuerySubscription.dispose();
  checkSetReady();
});
if (viewModel.design.id() > 0) {
  // Code mirror and ko.
...
% if query_id:
  var executeSubscription = viewModel.isReady.subscribe(function(value) {
    if (value) {
      viewModel.design.results.url('/' + viewModel.server() + '/results/${query_id}/0?format=json');
      viewModel.fetchResults();
      executeSubscription.dispose();
    }
  });
% endif
