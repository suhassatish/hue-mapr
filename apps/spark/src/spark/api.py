...
from spark.forms import SparkForm
...
def execute(request, design_id=None):
  ...
  #<QueryDict: {u'description': [u''], u'appName': [u'0'], u'classPath': [u'spark.jobserver.WordCountExample'], u'query': [u'WDWD'], u'id': [u'-1'], u'name': [u'']}>
...
#      query_history.save()
      ...
      try:
        print form.cleaned_data['appName'], form.cleaned_data['classPath'], form.cleaned_data['query']
        params = dict([param.split('=') for param in form.cleaned_data['query'].split('\n')])
        print params
        api = get_api(request.user)
        results = api.submit_job(form.cleaned_data['appName'], form.cleaned_data['classPath'], **params)        
        print results, type(results)
        #result = json.loads

#        if results['status'] == 'ERROR':
#          response['status'] = -1
#          response['errors'] = results['result']
#        else:
        response['status'] = 0
