...
class DocumentManager(models.Manager):
  ...
  def trashed(self, model_class, user):
    ...
    docs = Document.objects.get_docs(user).filter(content_type=ct).filter(tags__in=[tag]).order_by('-last_modified')[:100]
  ...
  def available(self, model_class, user):
    ...
    docs = Document.objects.get_docs(user).filter(content_type=ct).exclude(tags__in=[tag]).order_by('-last_modified')[:100]
