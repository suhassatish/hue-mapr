...
function sparkViewModel() {
  ...
  self.updateContexts = function(contexts) {
    ...
    $.each(contexts, function(index, value) {
        ...
        'nice_name': ko.observable(value)
      });
    });
    self.contexts(newContexts); 

    var last = $.totalStorage('hueSparkLastContext') || ((newContexts[0].length > 0) ? newContexts[0].name() : null);
    if (last) {
      self.context(last);
    }
  };
  ...
  self.modalUploadJar = function() {
    var request = {
      url: '/spark/api/upload',
      dataType: 'json',
      type: 'POST',
      success: function(data) {
        self.updateContexts(data.contexts);
