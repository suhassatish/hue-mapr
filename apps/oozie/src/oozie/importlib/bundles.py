...
def _set_controls(bundle, root, namespace):
  ...
  if kick_off_time:
    bundle.kick_off_time = kick_off_time[0].text
...
def _reconcile_coordinators(bundle, root, namespace):
  ...
  for coordinator_el in root.xpath('n:coordinator', namespaces=namespaces):
    try:
      coordinator = Coordinator.objects.get(name=coordinator_el.get('name'))
    except Coordinator.DoesNotExist:
      # @TODO(abe): Log or throw exception?
      pass
...
def import_bundle(bundle, bundle_definition):
